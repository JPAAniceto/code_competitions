...
int main(int argc, char **argv){
    ...
    char test_cases_buff[T_MAX];
    fgets(test_cases_buff, T_MAX ,(FILE*)f);
    ...
    for (int i =1 ; i <=nr_test_cases; i++){
        // grid dimention
        // scanf("%ld",&n);
        memset(n_buff, '\0', N_MAX);
        fgets(n_buff, N_MAX ,(FILE*)f);
        fflush(stdin);

        printf("%s\n", n_buff);
        fflush(stdout);

        n = string_to_lint(n_buff,get_string_size(n_buff));
        printf("%ld\n", n);
        fflush(stdout);
        ...
        memset(lydia_path_buff, '\0', size+1);
        fgets(lydia_path_buff, size+1 ,(FILE*)f);
        fflush(stdin);
