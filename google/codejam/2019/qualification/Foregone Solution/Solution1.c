...
int main(int argc, char **argv){
    ...
    char test_cases_buff[MAX_DIGITS_N];
    fgets(test_cases_buff, MAX_DIGITS_N ,(FILE*)f);
    nr_test_cases = test_cases_buff[0] - '0';
    ...
    for (int i =1 ; i <=nr_test_cases; i++){
        ...
        fflush(stdin);
        solutions[i] = (long int)(solve(test_case_buff, get_string_size(test_case_buff)));

        printf("Case #%d: %ld %ld\n", i,solutions[i], (long int)(string_to_lint(test_case_buff,get_string_size(test_case_buff)) - solutions[i]) );
        fflush(stdout);
    }
